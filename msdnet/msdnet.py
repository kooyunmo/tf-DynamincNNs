"""
An original PyTorch implementation is:
https://github.com/kalviny/MSDNet-PyTorch/blob/master/models/msdnet.py
This code is implemented with TensorFlow 2.2.0
"""
...
class ConvBasic(keras.models.Model):

    def __init__(self, nIn, nOut, kernel=3, stride=1, padding=1):
        super(ConvBasic, self).__init__()
        self.net = keras.Sequential([
            keras.layers.Conv2D(nOut, kernel_size=(kernel, kernel), 
...
class ConvBN(keras.models.Model):
    def __init__(self, nIn, nOut, type: str, bottleneck, bnWidth):
        ...
        if bottleneck is True:
            ...
            layer.append(keras.layers.Conv2D(nInner, kernel_size=(1, 1),
                                             strides=(1, 1), padding='same',
                                             use_bias=False))
            layer.append(keras.layers.BatchNormalization())
            layer.append(keras.layers.ReLU())

        if type == 'normal':
            layer.append(keras.layers.Conv2D(nOut, kernel_size=(3, 3),
                                             strides=(1, 1), padding='same',
                                             use_bias=False))
        elif type == 'down':
            layer.append(keras.layers.Conv2D(nOut, kernel_size=(3, 3),
        ...
        self.net = keras.Sequential(layer)

    def call(self, x):
        return self.net(x)


class ConvDownNormal(keras.models.Model):
    ...
    def __init__(self, nIn, nOut, args):
        ...
        if args.data.startswith('cifar'):
            ...
            conv = keras.Sequential([
                keras.layers.Conv2D(nOut * args.grFactor[0], kernel_size=(7, 7),
                                    strides=(2, 2), padding='same'),
                keras.layers.BatchNormalization(),
                keras.layers.ReLU(),
...
class MSDNLayer(keras.models.Model):
...
class ParallelModule(keras.models.Model):
    """
    ...
    def __init__(self, parallel_modules):
        '''
        @YM
        - parallel_modules: Python list
        '''
        super(ParallelModule, self).__init__()
        #self.m = nn.ModuleList(parallel_modules)
        self.m = list(parallel_modules)
...
class ClassifierModule(keras.models.Model):
...
class MSDNet(keras.models.Model):
    def __init__(self, args):
        ...
        self.blocks = list()    # nn.ModuleList()
        self.classifier = list()    # nn.ModuleList()
        ...
        '''
        # Unlike PyTorch, TensorFlow modules initialize the weights at constructions.
